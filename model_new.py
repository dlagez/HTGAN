...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=3, embed_dim=25, norm_layer=None):
        super().__init__()
...
class ADGANTransformer(nn.Module):
    r"""
    Args:
        img_size (int | tuple(int)): Input image size. Default 224
        patch_size (int | tuple(int)): Patch size. Default: 4
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=3, num_classes=17,
        ...
        self.conv1 = nn.Conv2d(in_chans, ndf, 4, 2, 1, bias=False)
...
class netD(nn.Module):
    ...
    def __init__(self, ndf, nc, nb_label):
        ...
        #self.disc_linear = nn.Linear(ndf * 2, 1)
        self.aux_linear = nn.Linear(192, nb_label+1)
        self.softmax = nn.LogSoftmax(dim=-1)
        #self.sigmoid = nn.Sigmoid()
        self.ndf = ndf
        self.apply(weights_init)
